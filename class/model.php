...
abstract class Model {
    ...
    public function save($item, $id = null) {
        ...
        if (empty($id)) {
            if ($this->exec('add', $item)) {
                ...
                //var_dump($res = $this->qb()->insert($this->table, $item)->debug('insert'));
                $id = $this->qb()->insert($this->table, $item)->prepare('insert')->execute()->lastInsertId();
                $this->log('added', $id, !empty($this->title) ? $item[$this->title] : '');
                return $id;
            }
        } else {
            if ($this->exec('save', $item)) {
                ...
                $this->log('edited', $id, !empty($this->title) ? $item[$this->title] : '');
                return true;
            }
        }
    }
        ...
        if (!empty($id)) {
            ...
            if (empty($this->fieldUser) || ($this->app->user->current['user_role'] == 'admin' || $item[$this->fieldUser] == $this->app->user->current['user_id'])) {
                if ($this->exec('delete', $item)) {
                    ...
                    $this->log('deleted', $id, !empty($this->title) ? $item[$this->title] : '');
                    return true;
                }
            } else {
                $this->app->error_503();
     ...
     * Add log entry
     */
    protected function log($action, $id, $title) {
        //ignore log actions
        if ($this->table == 'log')
            return true;
        $this->app->log->save(array(
            'log_module' => $this->table,
            'log_action' => $action,
            'log_target' => $title,
            'log_target_id' => $id,
